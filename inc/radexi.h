...
extern char radexi_path[PATH_MAX];
...
struct colpart
{
  enum ColPart  name;                         // collision partner's name defined as constant integer 
  double        dens;                         // collision partner's number density [cm-3]
  unsigned int  numof_coltr;                  // num of collisional transitions 
  unsigned int  numof_temps;                  // num of collisional temperatures
  ...
  gsl_matrix    *K;                           /* collisional rate coefficients (from database) [cm3 s-1].
                                                 It's dimensions are 'numof_radtr X numof_radtr', 
                                                 but need to allocate memory first.                     */
};
...
struct bg_field
{
  double        *intens;                      // background intensity [erg s-1 cm-2 Hz-1 sr-1]
};
...
struct enlev
{
  ...
  char          qnums[RXI_MAX_EXCESS_SIZE];   // quantum numbers in one string (writing excess strings here)
};
...
struct mol_info
{
  ...
  gsl_matrix    *C;                           /* collision rates per second for all collision partners [s-1].
                                                 Defined as 
                                                      C_ij += K_ij*ndens
                                                 To allocate memory needs numof_enlev dimensions.       */
  gsl_vector    *Ctot;                        /* total collision rate [s-1]. Defined as
                                                      Ctot_i = C_ij
                                                To allocate memory needs numof_enlev dimensions.        */
};
...
struct results 
{
  gsl_matrix    *rates;                       // rates from statistical equilibrium
  gsl_vector    *Tex;                         // excitation temperature [K]
  double        *tau;                         // optical depth for every line [cm-1]
...
struct rxi_data
{
  struct mc_par   mc;       /* Storing molecular clouds parameters here, which were mostly entered by the 
                               user. Additional info is written in collision partner's section, where
                               collisional coefficients and etc are defined.                            */
  struct mol_info mi;       /* Any information in here is written from database files. Pre-calculation 
                               is also done in here                                                     */
...
void start_dialogue (float *sfreq, float *efreq, struct rxi_input *inp);
...
int write_results(const float sfreq, const float efreq, const struct rxi_data *rxi, const char *result_path);
